...
def _save_protocol_rules_file(
    ...
    save_rules_file = bool(data.get("save_rules_file"))
    rules_output_dir = str(data.get("rules_output_dir") or "").strip()
    rules_file_name = str(data.get("rules_file_name") or "conversion_rules.json").strip() or "conversion_rules.json"

    if not save_rules_file:
        return None
...
def protocol_generate_rules():
    ...
    try:
        ...
        result = generate_protocol_field_rules(
            ...
            use_knowledge_base=bool(normalized_payload.get("use_knowledge_base", True)),
            use_page_index=bool(normalized_payload.get("use_page_index", False)),
        ...
        rules_json_path = _save_protocol_rules_file(normalized_payload, rules_payload)
        response_payload = {
            "conversion_rules_json": rules_payload,
            "kg_writeback_payload": result.get("kg_writeback_payload"),
            "_meta": {
                "rules_json_path": rules_json_path,
                "summary": result.get("summary"),
                "generated_rules": result.get("generated_rules"),
