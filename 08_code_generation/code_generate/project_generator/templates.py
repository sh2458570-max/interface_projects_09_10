...
def render_protocol_header(protocol: ProtocolSpec) -> str:
    ...
    for field in protocol.fields:
        default_value = str(field.default_value).strip() if field.default_value is not None else "0"
        if not _NUMERIC_LITERAL_RE.fullmatch(default_value):
            default_value = "0"
        field_lines.append(f"    long {field.cpp_name} = {default_value};")
...
def _render_codec_impl(protocol: ProtocolSpec, verify_spec: ProtocolVerifySpec | None = None) -> str:
    ...
    for section in protocol.sections:
            ...
            "\n".join(
                [
                    ...
                    *decode_lines,
                    "}",
                    "",
                    f"static void {write_section_name}({protocol.type_name}& value, QByteArray& data)",
                    "{",
                    *encode_lines,
                    "}",
                ]
        ...
        encode_section_calls.append(f"    {write_section_name}(value, data);")
