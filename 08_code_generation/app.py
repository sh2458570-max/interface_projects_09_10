...
def _load_local_code_generation_generate():
    ...
    spec = importlib.util.spec_from_file_location(
        "interface_project_08_local_api",
        LOCAL_APP_PATH,
    )
    ...
    return module.code_generation_generate


impl_code_generation_generate = _load_local_code_generation_generate()
...
def code_generation_generate():
    return impl_code_generation_generate()


@app.route("/health", methods=["GET"])
def health():
    return jsonify({"status": "healthy", "project": "08_code_generation"})


if __name__ == "__main__":
    app.run(host="0.0.0.0", port=6108, debug=True)
