...
def _extract_rule_items(payload: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(payload, dict):
        ...
        if isinstance(conversions, list):
            ...
            if items:
                return items
        for key in ("rules", "generated_rules", "target_field_rules"):
...
def evaluate_protocol_rules(
    ...
    field_match_accuracy = round(sum(field_match_values) / max(len(field_match_values), 1), 4)
    semantic_fidelity = round(sum(semantic_values) / max(len(semantic_values), 1), 4)
    structure_integrity = round(sum(structure_values) / max(len(structure_values), 1), 4)
    dimension_accuracy = round(sum(dimension_values) / max(len(dimension_values), 1), 4)
